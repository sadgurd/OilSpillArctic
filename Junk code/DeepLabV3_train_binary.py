...
def binarifyModel(model):
    ...
    new_weights = torch.add(torch.add(weights_0, weights_3), weights_4)
    new_bias = torch.add(torch.add(bias_0, bias_3), bias_4)
    ...
    Conv.weight[1].data = torch.add(weights_1, weights_2).to(torch.float16)

    Conv.bias[0].data = new_bias.to(torch.float16)
    Conv.bias[1].data = torch.add(bias_1,bias_2).to(torch.float16)

    # Replace the segmentation head in the model
    model.segmentation_head[0] = Conv.to(config.device)
...
if __name__ == '__main__':
    ...
    LossFunc = CrossEntropyLoss(weight=weights).to(config.device)
