...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_height = ((image.size(1) + 31) // 32) * 32
        target_width = ((image.size(2) + 31) // 32) * 32
        padding_height = target_height - image.size(1)
        padding_width = target_width - image.size(2)
        ...
        if self.T is False:
            pass
            #image = transforms.Pad((int(padding_width/2), int(padding_height/2)))(image)
